...
class PurchaseRequestPage(BasePage):
    ...
    SEARCH_DEPARTMENT = ".search-form .el-form-item:nth-child(4) .el-select"
    SEARCH_APPLICANT = "input[placeholder='请输入申请人']"
    SEARCH_STATUS = ".search-form .el-form-item:nth-child(6) .el-select"
    ...
    TABLE_HEADER_ROW = ".el-table__header-wrapper .el-table__row"
    CHECKBOX_ALL = ".el-table__header .el-checkbox"
    CHECKBOX_ROW = ".el-table__body .el-checkbox"
    PAGINATION = ".el-pagination"
    PAGE_NEXT = "button.btn-next"
    PAGE_PREV = "button.btn-prev"
    PAGE_SIZE_SELECT = ".el-pagination .el-select"
    PAGE_JUMPER = ".el-pagination .el-pagination__jump .el-input__inner"
    ...
    # 新增申请 - 申请信息表单
    FORM_APPLICANT = "[aria-label='新增采购申请'] input[disabled]"  # 申请人（只读）
    FORM_HANDLER = "[aria-label='新增采购申请'] .el-form-item:has-text('经办人') .el-select"
    FORM_REQUEST_NO = "input[placeholder='请输入申请编号']"
    FORM_DEPARTMENT = "[aria-label='新增采购申请'] .el-form-item:has-text('科室') .el-select"
    FORM_PROJECT = "[aria-label='新增采购申请'] .el-form-item:has-text('使用项目') .el-select"
    FORM_REQUEST_DATE = "[aria-label='新增采购申请'] .el-form-item:has-text('申请日期') input"
    FORM_EXPECTED_DATE = "[aria-label='新增采购申请'] .el-form-item:has-text('期望到货') input"
    FORM_URGENCY = "[aria-label='新增采购申请'] .el-form-item:has-text('紧急程度') .el-select"
    ...
    def navigate_via_menu(self):
        """通过左侧菜单导航：申请提交 -> 采购申请"""
        self.page.locator("text=申请提交").first.click()
        self.page.locator("text=采购申请").first.click()
    ...
    def get_cell_text(self, row: int, col: int) -> str:
        ...
        Args:
            ...
            col: 列索引，从0开始
        """
        selector = f"{self.TABLE_ROW}:nth-child({row + 1}) > td:nth-child({col + 1})"
        return self.inner_text(selector)
    ...
    def click_row_checkbox(self, row: int):
        ...
        Args:
            row: 行索引，从0开始
        """
        self.page.locator(self.TABLE_ROW).nth(row).locator(".el-checkbox").click()
    ...
    def click_view_button(self, row: int):
        """点击指定行的查看按钮"""
        self.page.locator(self.TABLE_ROW).nth(row).locator("button:has-text('查看')").click()

    def click_edit_button(self, row: int):
        """点击指定行的编辑按钮"""
        self.page.locator(self.TABLE_ROW).nth(row).locator("button:has-text('编辑')").click()

    def click_delete_button(self, row: int):
        """点击指定行的删除按钮"""
        self.page.locator(self.TABLE_ROW).nth(row).locator("button:has-text('删除')").click()

    # -------- 列表页 - 分页操作 --------
    def click_next_page(self):
        """点击下一页"""
        self.page.locator(self.PAGE_NEXT).click()

    def click_prev_page(self):
        """点击上一页"""
        self.page.locator(self.PAGE_PREV).click()

    def click_page_number(self, page_num: int):
        """点击指定页码"""
        self.page.locator(f".el-pager li:has-text('{page_num}')").click()
    ...
    def select_handler(self, handler: str):
        """在新增弹窗中选择经办人"""
        self.page.locator(self.FORM_HANDLER).click()
    ...
    def select_department(self, department: str):
        """在新增弹窗中选择科室"""
        self.page.locator(self.FORM_DEPARTMENT).click()
        self.page.get_by_role("listitem").filter(has_text=department).click()

    def select_project(self, project: str):
        """在新增弹窗中选择使用项目"""
        self.page.locator(self.FORM_PROJECT).click()
        self.page.get_by_role("listitem").filter(has_text=project).click()

    def fill_expected_date(self, date: str):
        """在新增弹窗中填写期望到货日期"""
        self.page.locator(self.FORM_EXPECTED_DATE).click()
        self.page.locator(self.FORM_EXPECTED_DATE).fill(date)
        self.page.keyboard.press("Enter")

    def select_urgency(self, urgency: str):
        """在新增弹窗中选择紧急程度"""
        self.page.locator(self.FORM_URGENCY).click()
        self.page.get_by_role("listitem").filter(has_text=urgency).click()
    ...
    def assert_search_result_contains(self, column: int, expected: str):
        """断言搜索结果中第一行指定列包含预期文本
        ...
        Args:
            ...
            expected: 预期文本
        """
        selector = f"{self.TABLE_ROW}:first-child > td:nth-child({column + 1})"
        self.assert_visible(f"{selector}:has-text('{expected}')")
