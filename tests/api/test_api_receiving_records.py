...
DATA_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "data", "api", "test_api_receiving_records_data.yaml")
query_data = ConfigReader(DATA_PATH)


@allure.feature("收货记录模块")
@allure.story("收货记录查询接口")
class TestApiReceivingRecords(BaseTest):
    """收货记录查询接口自动化测试用例"""

    API_RECEIVING_RECORDS_QUERY = "receiving_records_query"
    ...
    def test_query_default_success(self):
        ...
        response = self._api.send(
            self.API_RECEIVING_RECORDS_QUERY,
            params=data["params"],
            json_data=data["body"],
        )
        ...
        assert "list" in body["data"], "返回 data 中缺少 list 字段"
        self.logger.success("默认条件查询收货记录列表验证通过")

    @allure.title("按筛选条件查询收货记录成功")
    @allure.severity(allure.severity_level.NORMAL)
    @pytest.mark.api
    @pytest.mark.regression
    @pytest.mark.parametrize("case", [
        pytest.param("query_by_receiving_status", id="按收货状态"),
        pytest.param("query_by_purchase_status", id="按采购状态"),
    ...
    @allure.title("不携带 token 查询收货记录失败")
    @allure.severity(allure.severity_level.NORMAL)
    @pytest.mark.api
    @pytest.mark.regression
    def test_query_no_token(self):
        """不携带 Authorization 查询收货记录，验证接口拒绝访问"""
    ...
    @allure.title("携带无效 token 查询收货记录失败")
    @allure.severity(allure.severity_level.NORMAL)
    @pytest.mark.api
    @pytest.mark.regression
    def test_query_invalid_token(self):
        """携带无效 token 查询收货记录，验证接口拒绝访问"""
